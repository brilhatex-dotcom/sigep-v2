...
export default function AncoraLacre() {
  ...
  return (
    <div className="mb-3 rounded-lg border border-azul-frio bg-painel">
      <button onClick={() => setAberto(!aberto)} className="flex w-full items-center justify-between px-4 py-2.5 text-left">
        <span className="text-sm font-semibold text-ouro">⚓ Âncora externa do lacre</span>
        <span className="text-xs text-apagado">{selo ? `${selo.count} registros lacrados` : ""} {aberto ? "▲" : "▼"}</span>
      </button>
      {aberto && (
        <div className="border-t border-azul-frio px-4 py-3 text-sm text-texto-2">
          ...
          {selo && (
            <div className="mb-3 rounded-lg bg-campo p-3 font-mono text-[11px] leading-relaxed break-all">
          ...
          <div className="mb-3 flex flex-wrap gap-2">
            <button onClick={registrar} disabled={ocupado} className="rounded-lg bg-ouro px-3 py-1.5 text-xs font-semibold text-ouro-texto hover:brightness-110 disabled:opacity-60">
          ...
          <div className="mb-3">
            <label className="mb-1 block text-xs text-apagado">Conferir um selo salvo (cole o hash):</label>
            <div className="flex gap-2">
              <input value={hashConf} onChange={(e) => { setHashConf(e.target.value); setConfRes(null); }} placeholder="cole aqui o hash guardado..."
                className="flex-1 rounded-lg border border-borda bg-campo px-3 py-1.5 font-mono text-[11px] text-white outline-none focus:border-ouro" />
            ...
            <div>
              ...
              <ul className="space-y-1">
                {ancoras.slice(0, 6).map((a, i) => (
                  <li key={i} className="rounded bg-campo px-2 py-1 font-mono text-[10.5px] text-[#8fa3bf] break-all">
                    {dh(a.em)} · {a.count} reg · {a.por} · <span className="text-apagado-2">{(a.ultimoHash || "").slice(0, 24)}…</span>
                  </li>
                ))}
              </ul>
            </div>
